...
string version_check_url = "https://github.com/SilkieSabra/K-Bar-West/blob/master/flower/~version";
key github_version_request;

string news_url = "https://github.com/SilkieSabra/K-Bar-West/blob/master/flower/~news";
key news_request;
...
default
{
            ...
            if (iMenuIndex != -1)
            {
                ...
                if (sMenu=="Main"){
                    ...
                    if (sMessage == UPMENU) MainMenu(kAv, iAuth);
                        ...
                        g_kWebLookup = llHTTPRequest("https://github.com/SilkieSabra/K-Bar-West/blob/master/flower/~contact", [HTTP_METHOD, "GET", HTTP_VERBOSE_THROTTLE, FALSE], "");
                        g_kCurrentUser = kAv;
                        HelpMenu(kAv, iAuth);
    ...
    timer() {
        ...
        if (!g_iWillingUpdaters) {   //if no updaters responded, get upgrader info from web and remenu
            g_kWebLookup = llHTTPRequest("https://github.com/SilkieSabra/K-Bar-West/blob/master/flower/~update", [HTTP_METHOD, "GET", HTTP_VERBOSE_THROTTLE, FALSE], "");

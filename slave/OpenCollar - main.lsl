...
string version_check_url = "https://raw.githubusercontent.com/SilkieSabra/K-Bar-West/master/slave/~version";
key github_version_request;

string news_url = "https://raw.githubusercontent.com/SilkieSabra/K-Bar-West/master/slave/~news";
key news_request;
...
default
{
            ...
            if (iMenuIndex != -1)
            {
                ...
                if (sMenu=="Main"){
                    ...
                    if (sMessage == UPMENU) MainMenu(kAv, iAuth);
                        ...
                        g_kWebLookup = llHTTPRequest("https://raw.githubusercontent.com/SilkieSabra/K-Bar-West/master/slave/~contact", [HTTP_METHOD, "GET", HTTP_VERBOSE_THROTTLE, FALSE], "");
                        g_kCurrentUser = kAv;
                        HelpMenu(kAv, iAuth);
    ...
    timer() {
        ...
        if (!g_iWillingUpdaters) {   //if no updaters responded, get upgrader info from web and remenu
            g_kWebLookup = llHTTPRequest("https://raw.githubusercontent.com/SilkieSabra/K-Bar-West/master/slave/~update", [HTTP_METHOD, "GET", HTTP_VERBOSE_THROTTLE, FALSE], "");
